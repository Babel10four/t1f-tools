...
describe("LoanCalculatorClient", () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(() =>
        Promise.resolve(
          new Response(
            JSON.stringify({
              snapshot: { lastUpdated: "2026-08-03", dataThrough: "2026-07-31" },
              cities: [
                {
                  state: "FL",
                  regionId: 18142,
  ...
  it("loads a state market, calculates the workbook example, and renders eligibility", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/Market data through Jul 31, 2026/)).toBeInTheDocument();
    });
    await user.type(within(form).getByLabelText("City *"), "Tampa, FL");
    await user.selectOptions(within(form).getByLabelText("Borrower tier *"), "1");
    await user.type(within(form).getByLabelText("Guarantor FICO *"), "736");
    await user.type(within(form).getByLabelText("Flips completed with T1F *"), "3");
    ...
    expect(within(results).getByText("9.25%")).toBeInTheDocument();
    expect(within(results).getByText("8.75%")).toBeInTheDocument();
    expect(within(results).getAllByText("Eligible")).toHaveLength(2);
  });
