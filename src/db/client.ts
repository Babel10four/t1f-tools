...
export function getDb() {
  ...
  if (!url) {
    throw new Error(
      "DATABASE_URL is not set. This deployment needs Postgres for published rule bindings, the admin document library, and analytics. Set DATABASE_URL in your host (e.g. Vercel → Environment Variables), then run schema migrations (see README and drizzle/*.sql).",
    );
  }
  if (!db) {
